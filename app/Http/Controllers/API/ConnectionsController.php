...
class ConnectionsController extends Controller
{
    function getUserConnections(Request $request)
    {
        ...
        $user = User::with(['connections','connections.destUser','connections.sourceUser','connectors','connectors.sourceUser','connectors.destUser'])->where('uid', $uid)->first();
    ...
    function getUserWithDetails(Request $request, $uid)
    {
        ...
        if (User::find($uid)->exists()) {
            $user = User::where('uid', $uid)
                ->with("wallet")
                ->with(["referrals.user" => function ($query) {
